...
const Market = () => {
  return (
    <div className='flex flex-col md:flex-row w-full justify-center items-center gradient-bg-services px-10 py-20'>
            ...
            <div className='flex-1 flex flex-col justify-center border-2 border-gray-600 rounded-lg items-center'>
               {
                   marketData.map((item,idx) =>(
                      <>                      
                        <MarketItem key={item.ticker} {...item}/>
                        {idx < marketData.length - 1 && <div className='sm:w-[90%] w-full h-[0.25px] bg-gray-400 mb-2' />}
                      </> 
                   ))
               }
            </div>            
